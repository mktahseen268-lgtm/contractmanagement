...
| [`apps/api`](./apps/api) | **Backend** — FastAPI · SQLAlchemy · **PostgreSQL** (Alembic migrations · **Row-Level-Security multi-tenancy**) · JWT auth + refresh · contract lifecycle state machine · dashboard · append-only audit log · **Celery + Redis** background jobs (the OCR/AI pipeline). A runnable subset of the blueprint ("the spine"). |
| [`apps/web`](./apps/web) | **Frontend** — Next.js (App Router) · TypeScript · Tailwind, in the locked "Trust Workspace" skin (friendly azure blue, airy, soft-rounded). 3-pane shell, dashboard, contracts list/detail/create/edit, audit log, OCR/AI workspace, settings. |
| [`docker-compose.yml`](./docker-compose.yml) | Runs the whole stack: **Postgres + Redis + API + Celery worker + web**. |

> **Status:** a working, dynamic full-stack app — real PostgreSQL with Alembic migrations, database-enforced tenant isolation (RLS), real CRUD, a real lifecycle state machine, a real append-only audit log, and a real Celery/Redis-backed async OCR job. It's the *core spine* of the blueprint, built so the remaining modules slot in. It is **not** fully production-hardened yet (see "Going to production" below).
...
Brings up **Postgres** (`:5432`), **Redis** (`:6379`), the **API** (`:8000` — runs Alembic migrations + seeds a demo workspace on first start), a **Celery worker** (processes the OCR queue), and the **web app** (`:3000`).
...
│   ├── api/                       # FastAPI backend
│   │   ├── alembic.ini
│   │   ├── migrations/            # Alembic — env.py + versions/0001_initial.py + versions/0002_rls.py
│   │   ├── app/
│   │   │   ├── main.py            # app factory · CORS · routers · startup (run migrations + seed)
│   │   │   ├── config.py          # Pydantic settings (.env): DB url, Redis, Celery, JWT, CORS …
│   │   │   ├── database.py        # SQLAlchemy engine/session/Base + the RLS tenant-context plumbing
│   │   │   ├── models.py          # Tenant, User, Contract, ContractVersion, Comment, AuditLog, Notification, OcrJob
...
│   │   │   └── routers/           # auth · contracts · dashboard · audit · misc (users/notifications/ocr)
...
- **Storage**: S3 — TODO: presigned-URL uploads/downloads + a `files` table (currently the "document" is stored as text on the contract row; real PDFs/scans need S3 — docs/18).
