...
│   │   │   └── routers/           # auth (login/MFA/refresh/sessions/password) · contracts (incl. submit-for-approval / workflow/decide / pdf / versions / prepare-signature) · workflows · signatures (envelopes + the public /sign/{token} portal) · dashboard · audit · files · misc (users/notifications/ocr)
│   │   ├── requirements.txt · Dockerfile · .env.example
│   └── web/                       # Next.js frontend
│       ├── src/app/               # (auth)/login, login/mfa, register · (app)/dashboard, contracts/*, workflows/*, intelligence, audit, settings · (portal)/sign/[token]
...
- **Reports & analytics, billing, the external client portal, real-time presence, Arabic/RTL** — all designed in `docs/`, not yet built here.
